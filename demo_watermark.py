...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Single or comma separated list of the preprocessors/normalizer "
    ...
    parser.add_argument(
        ...
        help="Whether to use the detection method that only counts each unqiue bigram"
             " once as either a green or red hit.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="How to treat the permutation when selecting the greenlist tokens "
             "at each step. Legacy is (False) to pick the complement/reds first.",
    )
    parser.add_argument(
        "--skip_model_load",
        type=str2bool,
    ...
    # 是否加载为半精度模型（默认使用float32，精度高，占用空间大）
    # 在加载模型时加一层判断
    parser.add_argument(
        "--load_fp16",
        type=str2bool,
...
def generate(prompt, args, model=None, device=None, tokenizer=None, base_model=None):
    ...
    # batch_decode是比decode更通用的方法，单样本or批量样本均可处理
    # 生成结果是单样本，所以取第一个元素
...
#  构建并启动一个基于 Gradio 的交互式 Web 演示界面
